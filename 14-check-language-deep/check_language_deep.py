...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run deep language review checks without modifying files"
    )
    ...
    report = (
        Path(args.report)
        if args.report
        else project.reports_dir / "check_language_deep-report.json"
    )
    return run_language_deep_check(project, pack, report)
