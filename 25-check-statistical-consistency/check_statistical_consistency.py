...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check statistical notation consistency in final thesis source"
    )
    ...
    report = (
        Path(args.report)
        if args.report
        else project.reports_dir / "statistical-consistency-report.json"
    )
    return run_statistical_consistency_check(project, pack, report)
