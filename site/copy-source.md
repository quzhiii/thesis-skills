# Thesis Skills 页面文案源文件

这份文件是 `site/` 静态页面的文案源，保持与 `README.md`、`README.zh-CN.md` 和 `docs/roadmap.md` 的当前 public contract 一致。
...
### 副标题
Thesis Skills 是一套确定性、可检查的论文工作流系统，帮助研究者管理文献接入、Word/LaTeX 交接、检查修复、审阅闭环、readiness 判断、答辩前整理和规则包扩展。

### Badge
当前公开叙事：覆盖已落地 workflow 与明确边界
...
### 能力 6：规则包扩展
学校/期刊规则包创建、草稿脚手架、规则包 lint、schema consistency、scorecard 和最小导出交接包。
...
### 样例 2：Citation Integrity 报告包
本地引用完整性检查同时写出 `check_references-report.json` compatibility 报告，以及 `citation-integrity-report.json`、`citation-integrity-report.md`、`citation-issues.csv` 三种面向机器、人工和表格复核的产物。
...
### 样例 6：rule-pack lint scorecard / export bundle
检查 required files、metadata completeness、baseline completeness、schema consistency、overall status 和 finding counts。导出包 `manifest.json` 记录规则包元数据和 lint scorecard 摘要。
...
### 区块标题
当前公开版本明确支持什么，也明确不承诺什么

### v3.4.1 已包含
- EndNote / Zotero 文献导入与同步
- Word→LaTeX 迁移
- LaTeX→Word 审阅友好导出
- Citation Integrity JSON / Markdown / CSV 报告
- final reference set、外部验证、DOI / URL advisory 和 hallucination risk
- claim-citation support triage 和本地 HTML 报告界面（含 claim-citation P0 / P1 / P2 / P3 复核分组和 issue-card 风格摘要）
- final-audit JSON / HTML、reference audit ledger 和报告索引
- readiness / references / claim-citation / final-audit 深链，以及手机窄屏可读的本地 HTML bundle
- 引用、语言、格式、内容检查
- 深度语言筛查
- 编译日志结构化解析
...
2. **诚实不夸大**：明确展示当前版本的边界，不承诺尚未实现的功能。
