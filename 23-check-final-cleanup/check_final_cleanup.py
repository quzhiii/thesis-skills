...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check final thesis source for process residue before submission"
    )
    ...
    report = (
        Path(args.report)
        if args.report
        else project.reports_dir / "final-cleanup-report.json"
    )
    return run_final_cleanup_check(project, pack, report)
