...
from core.checkers import run_format_check
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check format and cross-reference integrity"
    )
    ...
    report = (
        Path(args.report)
        if args.report
        else project.reports_dir / "check_format-report.json"
    )
    return run_format_check(project, pack, report)
