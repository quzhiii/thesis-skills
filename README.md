...
The report also includes a backward-compatible support-review layer: `claim_type`, `support_review_label`, `support_review_reason`, `support_signals`, `risk_signals`, `cluster_keys`, `cluster_risk_summary`, and `next_actions`. These fields explain why a pair or grouped citation cluster deserves manual review; they do not replace the original `triage_label` or make final truth claims.
