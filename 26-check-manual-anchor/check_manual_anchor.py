...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check manual LaTeX contents lines for nearby phantomsection anchors"
    )
    ...
    report = (
        Path(args.report)
        if args.report
        else project.reports_dir / "manual-anchor-report.json"
    )
    return run_manual_anchor_check(project, pack, report)
