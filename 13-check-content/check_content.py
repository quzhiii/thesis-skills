...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check content structure and abstract metadata"
    )
    ...
    report = (
        Path(args.report)
        if args.report
        else project.reports_dir / "check_content-report.json"
    )
    return run_content_check(project, pack, report)
