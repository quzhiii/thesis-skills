...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check language style issues")
    ...
    report = (
        Path(args.report)
        if args.report
        else project.reports_dir / "check_language-report.json"
    )
    return run_language_check(project, pack, report)
