...
from core.checkers import run_bib_quality_check
from core.project import ThesisProject
from core.rules import find_rule_pack


def main() -> int:
    parser = argparse.ArgumentParser(description="Check exported bibliography quality")
    ...
    report = (
        Path(args.report)
        if args.report
        else project.reports_dir / "check_bib_quality-report.json"
    )
    return run_bib_quality_check(project, pack, report)
